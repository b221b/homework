...
WHERE clients.buy_date = '$date'";
            $firms_result = $conn->query($sql_firms);

            if ($firms_result->num_rows > 0) {
                while ($firm_row = $firms_result->fetch_assoc()) {
            ...
            WHERE models.name_firma = '$firm_name' AND buy_date = '$date') AS total_coast
        FROM postavshiki 
        WHERE name_firma = '$firm_name'";
                    ...
                    $sql_firm = "SELECT phone, email, website, city, (SELECT SUM(coast + podgotovka + transport_coast) 
                      ...
                      WHERE c.buy_date = '$date' AND m.id_postavshika = p2.id)
                      ) AS total_coast
                        FROM postavshiki p2
                        WHERE name_firma = '$firm_name'";
            ...
            } else { // this was the closing } bracket without corresponding opening
